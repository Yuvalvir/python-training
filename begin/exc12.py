...
def front_x(words):
    ...
    for i in words:
        if i.startswith('x'):
            ...
            list_2.append(i)
    return sorted(list_x) + sorted(list_2)
...
def remove_adjacent(nums):
    return_list = []
    for i in nums:
        if len(return_list) == 0 or i != return_list[-1]:
...
def linear_merge(sorted1, sorted2):
    ...
    while len(sorted1) != 0 and len(sorted2) != 0:
        if sorted1[0] < sorted2[0]:
            return_list.append(sorted1.pop())
        else:
            return_list.append(sorted2.pop())
    return_list.extend(sorted1)
    return_list.extend(sorted2)
    """
